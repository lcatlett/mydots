...
On `wip` branch. Unstaged changes exist in:
- `bin/dotfiles`, `dots/.zshrc` — core CLI and shell config
- `install/Brewfile`, `install/brew.sh`, `install/install.sh`, `install/symlinks.sh` — install pipeline

Staged deletions: `dots/.hushlogin` (removed).
Untracked: `editors/` directory.

The `master` branch is the stable baseline.
